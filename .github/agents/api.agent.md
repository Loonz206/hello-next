...
agents: ["React Agent", "Test Agent", "Lint Agent"]
target: vscode
handoffs:
  - label: Lint Validation (If API Code Changed)
    agent: Lint Agent
    prompt: "Let's validate the API implementation with linting."
    send: false
  - label: Component Integration
    agent: React Agent
    prompt: "Now let me review the React components that use these API patterns."
    send: false
  - label: Test Coverage
    agent: Test Agent
    prompt: "Now let's write comprehensive tests for these API hooks and integrations."
    send: false
---
