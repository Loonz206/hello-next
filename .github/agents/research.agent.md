...
target: vscode
handoffs:
  - label: Start Core Pipeline (Lint First)
    agent: Lint Agent
    prompt: "Code quality baseline. Let's lint and format the code first."
    send: false
  - label: React Component Review
    agent: React Agent
    prompt: "Now let's review React/Next.js patterns and component structure."
    send: false
  - label: API Data Patterns
    agent: API Agent
    prompt: "Let's implement proper React Query patterns for data fetching."
    send: false
  - label: Dependency Review
    agent: Package Agent
    prompt: "Let's analyze and verify all dependencies are appropriate."
    send: false
---
